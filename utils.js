...
function loadCertificates() {
    ...
    // ----- EXTRAIR CHAVE PRIVADA COMO PKCS#1 (RSA) -----
    let privateKeyPem = null;
    // Primeiro tenta PKCS#1 (keyBag)
    ...
    if (bags[forge.pki.oids.keyBag] && bags[forge.pki.oids.keyBag].length > 0) {
        ...
        if (bags[forge.pki.oids.pkcs8ShroudedKeyBag] && bags[forge.pki.oids.pkcs8ShroudedKeyBag].length > 0) {
            ...
            // Converte para PEM RSA (PKCS#1)
            privateKeyPem = forge.pki.privateKeyToPem(key);
            console.log('✅ Chave privada extraída (PKCS#8 Shrouded -> convertida para RSA)');
        } else {
            ...
            if (bags[forge.pki.oids.pkcs8KeyBag] && bags[forge.pki.oids.pkcs8KeyBag].length > 0) {
                ...
                privateKeyPem = forge.pki.privateKeyToPem(key);
                console.log('✅ Chave privada extraída (PKCS#8 -> convertida para RSA)');
            } else {
    ...
    // Log das primeiras linhas da chave privada (para diagnóstico)
