...
async function buscarValorExatoPagamento(orderId) {
    ...
    try {
        ...
        if (!orderId) {
            console.warn('⚠️ ID da venda inválido para buscar pagamento');
            return null;
        }

        console.log(`🔍 Buscando TODOS os pagamentos da venda ${orderId}...`);
        ...
        if (!orderResponse.ok) {
            console.warn(`⚠️ Não foi possível buscar venda ${orderId}: HTTP ${orderResponse.status}`);
            return null;
        }
        ...
        // =====================================================
        // LOCALIZAR TODOS OS PAYMENT IDS
        // =====================================================

        const paymentIds = [];

        // -----------------------------------------------------
        // FORMATO NORMAL DO ML
        // -----------------------------------------------------
        ...
        console.log(`💳 ${paymentIdsUnicos.length} pagamento(s) encontrado(s):`, paymentIdsUnicos);
        ...
        if (
            ...
            return {
                ...
                tipo_pagamento: null,
                metodo_pagamento_nome: 'Não informado',
                parcelas: null,
                parcelamento_nome: null,
                valor_parcela: null,
                pagamentos_detalhes: []
            };
        }
        ...
        for (
            ...
            try {
                ...
                console.log(
                    `💳 Consultando pagamento ${paymentId}...`
                );
                ...
                if (
                    ...
                    console.warn(`⚠️ Erro buscando pagamento ${paymentId}: HTTP ${paymentResponse.status}`);
                    continue;
                }
                ...
                console.log(`💳 Dados pagamento ${paymentId}:`, paymentData);

                // =============================================
                // IGNORAR PAGAMENTOS CANCELADOS/REJEITADOS
                // =============================================
                ...
                if (
                    ...
                    console.log(`ℹ️ Pagamento ${paymentId} ignorado por status: ${status}`);
                    continue;
                }
                ...
                // FRETE
                //
                // NÃO SOMAR FRETE ENTRE PAGAMENTOS.
                // Ele pode aparecer repetido.
                // =============================================
                ...
                console.warn(
                    `⚠️ Erro processando pagamento ${paymentId}:`,
                    error
                );
            }
        }
        ...
        // =====================================================
        // NENHUM PAGAMENTO CONSEGUIU SER CONSULTADO
        // =====================================================
        ...
        // CAMPOS RESUMIDOS
        //
        // Mantemos compatibilidade com o restante do sistema.
        // =====================================================
                ...
                pagamentosDetalhes
                    .map(
                        pagamento => {
                            ...
                            let texto =
                                `${pagamento.metodo_pagamento_nome} - R$ ${Number(
                                    pagamento.valor ||
                                    0
                                ).toFixed(2)}`;
        ...
        return {
            ...
            // =============================================
            // RESUMO COMPATÍVEL COM O SISTEMA ANTIGO
            // =============================================
        ...
        console.error(
            '❌ Erro ao buscar pagamentos no Mercado Pago:',
            error
        );
...
function consolidarSnapshotsEstoqueAnuncioNFE(
    lista
) {
...
function montarEstoqueAnuncioPosVendaHtmlNFE(
    venda
) {
...
function garantirEstruturaAvulsaNFE() {
    ...
    // =====================================================
    // CAMPO DE NF-E REFERENCIADA PARA DEVOLUÇÃO
    // =====================================================
...
async function preencherSelectCFOPAvulsaNFE() {
    ...
    try {
        ...
        select.disabled =
            ...
            // =====================================================
            // MOSTRAR/OCULTAR CHAVE REFERENCIADA
            // =====================================================
...
// =========================================================
// DEVOLUÇÃO - NF-E REFERENCIADA
// =========================================================

function normalizarTextoAvulsaNFE(valor) {
...
// =========================================================
// VERIFICA SE A NATUREZA É ENTRADA DE DEVOLUÇÃO
// =========================================================

function ehEntradaDevolucaoAvulsaNFE() {
...
// =========================================================
// GARANTIR CAMPO DA CHAVE REFERENCIADA
// =========================================================

function garantirCampoReferenciaDevolucaoAvulsaNFE() {
    ...
    let grupo =
        document.getElementById(
            'avulsaReferenciaDevolucaoGrupo'
        );
    ...
    grupo.innerHTML = `
        ...
        <label
            for="avulsaChaveNFeReferenciada"
            style="
                display:block;
                margin-bottom:7px;
                font-weight:500;
            "
        >
            Chave de acesso da NF-e de saída *
        </label>

        <input
            type="text"
            id="avulsaChaveNFeReferenciada"
            class="form-control"
            maxlength="44"
            inputmode="numeric"
            autocomplete="off"
            placeholder="Digite os 44 números da chave da NF-e original"
        >
        ...
        <small
            ...
            "
        >
            Informe a chave da NF-e de saída referente a esta devolução.
        </small>

        <div
            id="avulsaChaveNFeReferenciadaContador"
            style="
                margin-top:4px;
                font-size:12px;
                color:#6c757d;
    ...
    // =====================================================
    // COLOCAR LOGO DEPOIS DA NATUREZA
    // =====================================================
    ...
    // Caso o HTML não use .form-group
    if (!grupoNatureza) {
    ...
    if (
        grupoNatureza &&
        grupoNatureza.parentElement
    ) {

        grupoNatureza.insertAdjacentElement(
            'afterend',
            grupo
        );
        ...
        // =================================================
        // FALLBACK:
        // colocar antes da modalidade de frete
        // =================================================
    ...
    if (input) {
        ...
        input.oninput =
            function () {
                ...
                const contador =
                    document.getElementById(
                        'avulsaChaveNFeReferenciadaContador'
                    );
                ...
                if (contador) {
                    ...
                    contador.textContent =
                        `${this.value.length} / 44 dígitos`;


                    contador.style.color =
                        this.value.length === 44
                            ? '#198754'
                            : '#6c757d';
                }
            };
    ...
    console.log(
        '✅ Campo de NF-e referenciada criado'
    );
...
function atualizarCampoReferenciaDevolucaoAvulsaNFE() {
    ...
    const ehDevolucao =
        ehEntradaDevolucaoAvulsaNFE();


    console.log(
        '🔎 Natureza NF-e avulsa:',
        document
            .getElementById(
                'avulsaNatOp'
            )
...
// =========================================================
// NF-E AVULSA - REFERÊNCIA DE DEVOLUÇÃO
// =========================================================

function normalizarNaturezaAvulsaNFE(valor) {
...
function garantirCampoReferenciaDevolucaoAvulsaNFE() {
    ...
    if (!grupo) {
        ...
        grupo.innerHTML = `
            ...
            <label for="avulsaChaveNFeReferenciada">
                Chave de acesso da NF-e de saída *
            </label>
            ...
            <small
                ...
                "
            >
                Informe a chave da NF-e original que está sendo devolvida.
            </small>
...
async function prepararEmissaoAvulsaNFE() {
    ...
    try {
        ...
        const resultados =
            ...
            // =================================================
            // GARANTIR CAMPO DE REFERÊNCIA DA DEVOLUÇÃO
            // =================================================
...
function agruparVendasEmPacksNFE(
    ...
    for (
        ...
        const responsaveisBaixaUsername =
            [
                ...new Set(
        ...
        const algumRestauradoSyncPendente =
            membros.some(
                venda =>
                    venda
                        ._estoque_status ===
        ...
        // =====================================================
        // ESTOQUE DOS ANÚNCIOS APÓS A VENDA
        // =====================================================
        ...
        const agrupada = {
            ...
            _estoque_baixado_por_username:
                responsaveisBaixaUsername.join(
                    ' / '
                ) ||
                null,

            _estoque_baixado_por_nome:
                responsaveisBaixaNome.join(
                    ' / '
                ) ||
                null,
...
window.verificarCancelamentosVendasNFE =
    ...
    // =========================================================
// USUÁRIO RESPONSÁVEL POR AÇÕES NA NF-E
// =========================================================

function obterUsuarioOperacaoNFE() {
...
async function buscarEstoqueAnuncioPosVendaNFE(
    venda
) {
    ...
    if (
        ...
        console.warn(
            '⚠️ Token ML não disponível para consultar estoque pós-venda'
        );
...
if (
    ...
    const htmlPagamento =
        venda => {
            ...
            const pagamentosDetalhes =
                Array.isArray(
                    venda._pagamentos_detalhes
                )
                    ? venda._pagamentos_detalhes
                    : (
                        Array.isArray(
                            venda.pagamentos_detalhes
                        )
                            ? venda.pagamentos_detalhes
                            : []
                    );
...
// =================================================
// VERIFICAR SE O SNAPSHOT ATUAL É VÁLIDO
// =================================================
...
if (isFull) {
    ...
    // =================================================
    // DADOS DE PAGAMENTO JÁ EXISTENTES
    // =================================================

    let metodoPagamentoIdFull =
        venda._metodo_pagamento_id ??
        venda.metodo_pagamento_id ??
        metodoPagamentoId ??
        null;
    ...
    // =================================================
    // BUSCAR PAGAMENTO TAMBÉM PARA FULL
    // =================================================

    try {

        console.log(
            `💳 [FULL] Buscando pagamento da venda ${idVenda}...`
        );
        ...
        if (pagamento) {
            ...
            pagamentosDetalhesFull =
                Array.isArray(
                    pagamento.pagamentos_detalhes
                )
                    ? pagamento.pagamentos_detalhes
            ...
            console.log(
                `✅ [FULL] Pagamento processado da venda ${idVenda}:`,
                {
                    metodoPagamentoId:
                        metodoPagamentoIdFull,
        ...
        // =================================================
        // ERRO DE PAGAMENTO NÃO DEVE IMPEDIR VENDA FULL
        // =================================================

        console.warn(
            `⚠️ [FULL] Erro buscando pagamento da venda ${idVenda}:`,
            error
        );
    }
    ...
    return {
        ...
        // ESTOQUE
        //
        // FULL NÃO TEM BAIXA NO ESTOQUE LOCAL
        // =============================================
...
async function confirmarEmissaoNFEInterna() {
    ...
    const orderIdPrincipal =
        ...
        // =====================================================
        // USUÁRIO QUE ESTÁ EMITINDO A NF-E
        // =====================================================

        const usuarioEmissao =
                    ...
                    const {
                        ...
                        await window
                            ...
                            .insert({
                                ...
                                emitido_por_username:
                                    usuarioEmissao.username,
...
async function garantirBaixaEstoqueVenda(
    ...
    try {
        ...
        // 3. JÁ BAIXADO
        //
        // NÃO alterar responsável original.
        // =====================================================
        ...
        // =====================================================
        // OUTRA EXECUÇÃO JÁ FEZ A BAIXA
        //
        // NÃO registrar usuário atual.
        // =====================================================
        ...
        // =====================================================
        // 7. REGISTRAR IMEDIATAMENTE QUEM DEU BAIXA
        // =====================================================
        ...
        if (
            ...
            console.warn(
                `⚠️ Baixa feita, mas não foi possível salvar o responsável da venda ${vendaId}:`,
                erroResponsavel
            );
        }
...
nfes.forEach(
    nota => {
        ...
        try {
            ...
            // =================================================
            // IDENTIFICAR SE ESTA NOTA É DEVOLUÇÃO
            // =================================================
            ...
            // =================================================
            // PEGAR A CHAVE ORIGINAL REFERENCIADA
            // =================================================
...
const chaveNFeReferenciada =
    ...
    // =====================================================
// DEVOLUÇÃO PRECISA REFERENCIAR A NF-E ORIGINAL
// =====================================================

if (
    entradaDevolucao &&
    chaveNFeReferenciada.length !==
        44
) {

    showToast(
        'Informe os 44 dígitos da chave de acesso da NF-e de saída que está sendo devolvida.',
        'warning'
    );
...
// =====================================================
// LER RESPOSTA COMPLETA DO BACKEND
// =====================================================
...
if (
    ...
    throw new Error(
        `HTTP ${response.status}: ${detalheErro}`
    );
}
...
async function preencherSelectNaturezaNFE(
    ...
    try {
        ...
        console.log(`✅ ${naturezas.length} Natureza(s) carregada(s) para ${contexto}`,
            {
                padrao:
                    padrao?.descricao
            }
        ...
        console.error('❌ Erro carregando Naturezas da Operação:', error);
...
window.normalizarTextoAvulsaNFE = normalizarTextoAvulsaNFE;
window.ehEntradaDevolucaoAvulsaNFE = ehEntradaDevolucaoAvulsaNFE;
