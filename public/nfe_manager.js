// nfe_manager.js - Versão corrigida (sem conflitos globais)
(function() {
    // Garantir que WORKER_URL não seja redeclarado
    if (!window.WORKER_URL) {
        window.WORKER_URL = 'https://purple-bonus-3b1c.andmiotto1998.workers.dev';
    }
    if (!window.API_BASE_URL) {
        window.API_BASE_URL = 'https://backend-nfe.onrender.com';
    }
    ...
    async function carregarVendasPendentes() {
        ...
        try {
            ...
            if (!token) throw new Error('Token ML não disponível. Faça login novamente.');
            ...
            if (results.length === 0) {
                tbody.innerHTML = '<td><td colspan="6" class="text-center">Nenhuma venda encontrada nos últimos 30 dias<\/td><\/tr>';
                return;
            }
...
async function confirmarEmissaoNFE() {
    ...
    async function carregarTransportadoras() {
        ...
        try {
            ...
            console.error(error);
            tbody.innerHTML = '</table><td colspan="4" class="text-center text-danger">Erro ao carregar transportadoras<\/td><\/tr>';
        }
    }
